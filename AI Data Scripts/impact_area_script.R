...
if(is.na(inputFile) || length(inputFile) == 0)
  inputFile <- "data/impact_area.csv"
#####################################################################################
...
#####################################################################################
impactArea <- read.csv(inputFile)
IDMap <- dbGetQuery(con, "Select discourse_id, ori_id as id from Discourse;")
DiscourseImpactArea <- join(x = IDMap, y = impactArea, by="id", type = "inner")
DiscourseImpactArea$id <- NULL
DiscourseImpactArea$model_id <- 2
createSQLInserts("DiscourseImpactArea", DiscourseImpactArea)
dbDisconnect(con)
print("done!")
